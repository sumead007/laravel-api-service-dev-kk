...
@section('content')
    ...
    <div class="content-wrapper">
        ...
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">จัดการข้อมูลลูกค้า</h1>
                    </div><!-- /.col -->
                    <div class="col-sm-6">
    ...
    <div class="modal fade" id="CodeModal" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title" id="text_addcus">Code</h4>
                </div>
                <div class="modal-body">
                    <form name="form_third" id="form_third" class="form-horizontal">
                        <input type="hidden" name="code_id" id="code_id">
                        <textarea class="form-control" id="code" name="code" style="min-width: 100%"></textarea>
                    </form>
                </div>
                <div class="modal-footer">
                    {{-- <button type="button" class="btn btn-outline-danger mr-auto" id="btn_user_status"
                        onclick="user_status(event.target)">ปิดบัญชีนี้</button> --}}
                    <button type="button" class="btn btn-primary" onclick="save_code()">บันทึก</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <script>
        function editPost(pass_id) {
            clear_ms_error()
            var id = pass_id;
            let _url = "/admin/manage/customer/get_post/" + id;
        ...
        function detail(pass_id) {
            clear_ms_error()
            var id = pass_id;
            let _url = "/admin/manage/customer/get_code/" + id;
            $("#text_addcus").html("แก้ไขรายเบอร์โทร");
            $("#form_second")[0].reset();
            $.ajax({
                url: _url,
        ...
        function save_code() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    var form = $('#form_third')[0];
                    var data = new FormData(form);
                    var id = $("#post_id").val();
                    let _url = "{{ route('admin.manage.customer.save_code') }}";
                    $.ajax({
                        url: _url,
                        type: "POST",
                        headers: {
        ...
        function createPost() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    var data = new FormData(form);
                    var id = $("#post_id").val();
                    let _url = "{{ route('admin.manage.customer.store') }}";
                    $.ajax({
                        url: _url,
                        type: "POST",
                        headers: {
